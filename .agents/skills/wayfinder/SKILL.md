...
1. **Consult the atlas — only when `docs/atlas/` exists.** Read the index, then the area this effort sits in, and inherit what the atlas already settled: its out-of-scope rulings are this map's out-of-scope rulings, and its edges say what this area is blocked by. Carry them in before naming the destination, so the map doesn't rediscover them mid-flight. This map serves one effort and ends; the atlas doesn't — nothing here writes back to it. No `docs/atlas/`: skip this step and chart as usual.
