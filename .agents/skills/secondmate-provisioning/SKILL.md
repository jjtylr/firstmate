...
For every route, an explicit per-spawn `--harness` flag or positional harness argument must be one exact verified canonical adapter token and starts clean on model and effort, unless the caller also passes explicit `--model` or `--effort`.
Raw, wrapped, compound, assignment-driven, and otherwise executable harness strings are refused before launch.
