...
3. **Branch — is this a multi-session build?**
   ...
   How big should one spec be? [SIZING.md](../to-spec/SIZING.md) — a path relative to **this file** — is the single home for the sizing rule. Read it at this branch, before `/to-spec`, so an oversized spec is caught before work starts. If the `to-spec` skill is not installed beside this one that file is absent; say so rather than reciting a rule from memory.
...
- **Project-level orientation — "where do I start", "what's the horizon", "what would this feature or pivot touch"** → **`/atlas`**, but **only when this repo has `docs/atlas/`**. Look before you route; the atlas is opt-in per repo. It is the standing project map: it orients from the map, reads the impact of a proposed feature or pivot off the dependency graph, and ends by carving the next spec-sized bite — handing it to **`/wayfinder`** when that area is still foggy, or to **`/grill-with-docs`** → **`/to-spec`** when it is one spec's worth. Once a repo has an atlas, open it instead of cold-starting a grill on a project question. **No `docs/atlas/`? Every route below is exactly as it was** — route the orientation ask by its own shape, to `/grill-with-docs` or `/wayfinder`.
