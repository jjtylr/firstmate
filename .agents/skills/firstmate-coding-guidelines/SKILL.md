...
Every new Firstmate-owned skill needs its load trigger declared inline: section 13 for agent-only reference skills, or the relevant operating section for anything else.
For toolkit-installed internal skills, the generated available-skills catalog and the skill's frontmatter description own the trigger instead, so `AGENTS.md` must not duplicate it.
