...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" "$sub_state/$child_id.progress" \
      "$sub_state/$child_id.pi-ext.ts" "$sub_state/$child_id.pi-ready" \
      "$sub_state/$child_id.pi-ready.tmp."* "$sub_state/$child_id.omp-ext.ts" \
...
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.progress" \
  "$STATE/$ID.pi-ext.ts" "$STATE/$ID.pi-ready" "$STATE/$ID.pi-ready.tmp."* \
  "$STATE/$ID.omp-ext.ts" "$STATE/$ID.grok-turnend-token" \
